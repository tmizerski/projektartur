...
const queryBuilder = (filterModel) => {
    ...
    const query = ` select numer, m_symbol, data_spz_oper, wart_net_pln, ilosc_jme from sp_sprzedaz order by data_spz_oper desc  OFFSET 5 ROWS FETCH NEXT 5 ROWS ONLY`;
    ...
    if(!filterModel.setFilter) {
        ...
        if(filterModel.symbolOr) {
            multiFilterFlag ?  filter += ` and m_symbol = '${filterModel.filterValue.symbol}' or m_symbol = '${filterModel.filterValue.symbolSecond}'  ` : filter += ` m_symbol = '${filterModel.filterValue.nettoTo}' or symbol = '${filterModel.filterValue.symbolSecond}' `;
            if(!multiFilterFlag) return multiFilterFlag = !multiFilterFlag;
        }
        if(filterModel.symbolAnd) {
            multiFilterFlag ?  filter += ` and m_symbol = '${filterModel.filterValue.symbol}' and m_symbol = '${filterModel.filterValue.symbolSecond}'  ` : filter += ` m_symbol = '${filterModel.filterValue.nettoTo}' and symbol = '${filterModel.filterValue.symbolSecond}' `;
            if(!multiFilterFlag) return multiFilterFlag = !multiFilterFlag;
        }
        ...
        const queryFilter = ` select numer, m_symbol, data_spz_oper, wart_net_pln, ilosc_jme from sp_sprzedaz where ${filter} order by data_spz_oper desc  OFFSET 5 ROWS FETCH NEXT 5 ROWS ONLY`;
